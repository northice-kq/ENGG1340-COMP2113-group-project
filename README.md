...
-Player needs to explore the grid, encounters enemies, and interacts with different types of rooms such as combat, chest, and key rooms.
...
(5) Multiple files: Implemented across Map.cpp, Map.h, and room.cpp.
...
Enemy stats scale based on progression through the dungeon.
Hard mode introduces additional mechanics such as dodge-based reaction timing and random combat events.
...
(3) Dynamic memory management: Weapons and combat effects use dynamic allocation (new Weapon, new Healing).
...
(3) Dynamic memory management: Items are dynamically allocated using pointers (new).
...
No non-standard libraries are used in this project.
All functionalities are implemented using standard C++ libraries only, including:
...
Notes
The game must be compiled using a C++ compiler.
The program is designed to run on the Linux server environment.
If compilation fails, ensure all .cpp and .h files are included in the Makefile.
